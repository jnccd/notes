...
public class MainActivity : AvaloniaMainActivity
{
    ...
    void UpdateWidget()
    {
        try
        {
            ...
            if (widgetText == null)
            {
                // Nothing to show yet (typical right after startup, before the view is loaded and
                // the config is read); keep whatever the widget currently displays. Logged so the
                // app log tells this apart from a worker that could not fetch anything at all.
                WidgetDataRepository.Log($"in-app update had no displayable notes ({app.MainViewModel.VirtualRoot.SubNotes?.Count ?? 0} top level) - keeping the displayed text");
                return;
            }

            WidgetDataRepository.SaveData(this, widgetText);
            WidgetDataRepository.RequestUpdate(this);
            WidgetDataRepository.Log($"in-app update from {app.MainViewModel.VirtualRoot.SubNotes?.Count ?? 0} top level note(s), {widgetText.Length} chars");
        }
        catch (Exception ex)
        {
